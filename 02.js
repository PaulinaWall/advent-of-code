...
policys.forEach((policy) => {
	const arrayOfMinAndMaxNumbers = policy[0].split('-');
	const objectKey = Object.keys(policy[1]);
	const objectValue = Object.values(policy[1]);

	const minNumber = Number(arrayOfMinAndMaxNumbers[0]);
	const maxNumber = Number(arrayOfMinAndMaxNumbers[1]);
	
	let includesLetterTimes = 0;
	for (let i = 0; i < objectValue[0].length; i ++) {
		if ( objectValue[0].charAt(i) === objectKey[0] ) {
			includesLetterTimes ++;
		}
	}
	if(includesLetterTimes >= minNumber && includesLetterTimes <= maxNumber) {
		validPasswords ++;
	}
});
console.log(validPasswords);
// Answer 1:1 validPasswords = 467
...
policys.forEach((policy) => {
	...
	if ( objectValue[0].charAt(minNumber - 1) === objectKey[0] && objectValue[0].charAt(maxNumber - 1) !== objectKey[0]) {
		validPasswords2 ++;
	}else if(objectValue[0].charAt(minNumber - 1) !== objectKey[0] && objectValue[0].charAt(maxNumber - 1) === objectKey[0]){
		validPasswords2 ++;
	}
});

console.log(validPasswords2);
// Answer 1:2 validPasswords = 441